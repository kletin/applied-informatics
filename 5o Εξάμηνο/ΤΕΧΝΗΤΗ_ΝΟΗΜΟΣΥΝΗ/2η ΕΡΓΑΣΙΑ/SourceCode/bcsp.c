...
#define TIMEOUT		30		// Program terminates after TIMOUT secs
...
void print_vector(int *vector,char *msg) {
    printf("--- %s ---- \n",msg);
    int i;
    for(i=0;i<N;i++)
        printf("%d ",vector[i]);
...
void print_head() {
    printf("--- head ---- \n");
    struct stack_item *temp=head;
    while(temp) {
        print_vector(temp->vector,"head");
        temp=temp->next;
    }
}
...
int generate_children(int *vector)
{
	...
	if (valid(vector))				// Check whether a "false" assignment is acceptable...
	{
        ...
        //print_vector(vector,"push child 1");
		if (err==0)
			return 0;
	}
	...
	{
		err=push(vector);			// ...and push to the stack if yes.
        //print_vector(vector,"push child 2");
		if (err==0)
			return 0;
	}

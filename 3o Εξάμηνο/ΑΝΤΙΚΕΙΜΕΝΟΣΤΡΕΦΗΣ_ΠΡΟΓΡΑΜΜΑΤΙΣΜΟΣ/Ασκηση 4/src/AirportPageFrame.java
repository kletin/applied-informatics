...
public class AirportPageFrame extends JFrame {
	...
	public AirportPageFrame(Airport airport) {
		...
		findFlightsButton.addActionListener(new ActionListener() {
		    ...
		    public void actionPerformed(ActionEvent e) {
		    	...
		    	Airport destinationAirport = centralRegistry.getAirport(t6.getText());
		    	

		    	Airport airportΒ = CentralRegistry.getAirport(t6.getText());
		    	System.out.println(destinationAirport.getName());
		    	System.out.println(airport.getName());
		    	if(destinationAirport.equals(airport)) {
					JOptionPane.showMessageDialog(AirportPageFrame.this, "Arrival and departure city cannot be the same");		    		
		    		
		    	}
		        area3.setText(CentralRegistry.getDirectFlightsDetails(airport, airportΒ));
		        
		        
		        area4.setText(CentralRegistry.getInDirectFlightsDetails(airport, destinationAirport));
		        
		        try {
                    createFlightsFile(airport.getName(),
                    		CentralRegistry.getDirectFlightsDetails(airport, airportΒ),
                    		CentralRegistry.getInDirectFlightsDetails(airport, destinationAirport));
                    String airportCode = t6.getText();
                    Airport destinationAirport1 = centralRegistry.getAirport(airportCode);
	...
	public static String convertArrayListToString(ArrayList<String> arrayList) {
	  ...
	  private void createFlightsFile(String airport,String directFlights,String inDirectFlights) throws IOException {
	        String fileName = "FlightsInfo.txt";

	        try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
	            writer.println("CITY: " +t3.getText()+ ", " +t4.getText());
	            writer.println("AIRPORT: " +t1.getText()+" (" +t2.getText()+ ")") ;
	            writer.println("\n");
	            writer.println("DESTINATION: "+t6.getText());
	            writer.println("\n");

	            
	            writer.println(directFlights);
	            writer.println(inDirectFlights);
	    ...
	    private void openFlightsFile() throws IOException {
	        File file = new File("FlightsInfo.txt");
	        if (file.exists()) {
	            Desktop.getDesktop().open(file);
	        } else {
	            JOptionPane.showMessageDialog(this, "Flights file not found", "Error", JOptionPane.ERROR_MESSAGE);
	        }
	    }
}

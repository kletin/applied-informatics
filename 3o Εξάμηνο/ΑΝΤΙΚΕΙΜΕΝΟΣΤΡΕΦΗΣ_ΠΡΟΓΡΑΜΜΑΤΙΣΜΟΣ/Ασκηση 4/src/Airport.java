...
public class Airport {
	...
	public void addAirline(String airline) {
		...
		if (!airlines.contains(airline)) {
			// If it doesn't exist, add the airline
			airlines.add(airline);
			// System.out.println("Airline added: " + airline);
		} else {
			// If it already exists, print a message or handle accordingly
			// System.out.println("Airline already exists: " + airline);
			// Alternatively, you can throw an exception, log a message, etc.
		}
	}
	...
	public boolean isInDirectlyConnectedTo(Airport anAirport) {
		// Delegates the call to the recursive method with a new HashSet for visited
		// airports and depth set to 0.
		return isInDirectlyConnectedTo(anAirport, new HashSet<>(), 0);
	}

	/**
	 * <This method checks if this airport is indirectly connected to another
	 * airport within a certain depth limit.>
	 * 
	 * @param <anAirport>       The airport to check for indirect connection.
	 * @param <visitedAirports> Set of airports already visited to avoid infinite
	 *                          loops.
	 * @param <depth>           The current depth of recursion to limit the search
	 *                          depth.
	 * @return <true> if there is an indirect connection within the specified depth

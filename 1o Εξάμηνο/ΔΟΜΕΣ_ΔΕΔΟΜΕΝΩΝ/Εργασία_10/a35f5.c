...
BinTreePointer LCANode(BinTreePointer Root, BinTreePointer node1, BinTreePointer node2) {
  ...
  // If both node1 and node2 are smaller than Root, then LCA lies in left
  if (Root->Data > node1->Data && Root->Data > node2->Data)
    return LCANode(Root->LChild, node1, node2);

  // If both node1 and node2 are greater than Root, then LCA lies in right
  if (Root->Data < node1->Data && Root->Data < node2->Data)
...
int main(void) {
  BinTreePointer ARoot, node1, node2,lcaNode;
  ...
  lcaNode = LCANode(ARoot,node1,node2);
  printf("LCA node of %d and %d is node with element %d\n",num1,num2,lcaNode->Data);
